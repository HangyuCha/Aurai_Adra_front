...
import steps from './KakaoAddByPhonePracticeSteps.js';
...
export default function KakaoAddByPhonePractice(){
  ...
  const hintStorageKey = `practiceHintCount:kakao:addByPhone`;
  ...
  async function submitPractice(){
    ...
    if(finalizeSave){
      try{
        ...
        setResult(res); try{ localStorage.setItem('practiceScore:kakao:addByPhone', JSON.stringify(res?.score ?? null)); } catch { /* ignore */ }
        return;
    ...
    try{
      ...
      setResult({ score, elapsed: (score?.derived?.elapsedSec ?? null) }); try{ localStorage.setItem('practiceScore:kakao:addByPhone', JSON.stringify(score ?? null)); } catch { /* ignore */ }
  ...
  const forceShowResult = useCallback(async ()=>{
    ...
    try{ tracker && typeof tracker.end === 'function' && tracker.end(); } catch { /* ignore */ }
    try{ flushCompositionRef.current && flushCompositionRef.current(); } catch { /* ignore */ }
    if(finalizeSave){ try{ const res = await finalizeSave(); try{ const finalMs = startedAtRef.current ? (Date.now()-startedAtRef.current) : (res?.score?.derived?.elapsedSec? (res.score.derived.elapsedSec*1000):0); const finalSec = Math.round((finalMs/10))/100; if(res && res.score) res.score.derived = { ...(res.score.derived||{}), elapsedSec: finalSec }; } catch { /* ignore */ } setResult(res); try{ localStorage.setItem('practiceScore:kakao:addByPhone', JSON.stringify(res?.score ?? null)); } catch { /* ignore */ } return; }catch{ console.debug('finalizeSave failed in forceShowResult'); } }
    try{ const score = tracker?.scoreNow ? tracker.scoreNow() : null; try{ const finalMs = startedAtRef.current ? (Date.now()-startedAtRef.current) : (score?.derived?.elapsedSec? (score.derived.elapsedSec*1000):0); const finalSec = Math.round((finalMs/10))/100; if(score) score.derived = { ...(score.derived||{}), elapsedSec: finalSec }; } catch { /* ignore */ } setResult({ score, elapsed: (score?.derived?.elapsedSec ?? null) }); try{ localStorage.setItem('practiceScore:kakao:addByPhone', JSON.stringify(score ?? null)); } catch { /* ignore */ } }catch(e){ console.debug('forceShowResult fallback failed', e); setResult({ score: { total:0, breakdown:{}, derived:{ elapsedSec } }, elapsed: (elapsedSec ?? null) }); }
  ...
  try{ if(t && t.closest && t.closest('button[aria-label*="힌트"]')) return; } catch (e) { void e; }
  try{ if(t && t.closest && (t.closest('[aria-label="가상 키보드"]') || t.closest('[role="application"][aria-label="가상 키보드"]'))) return; } catch (e) { void e; }
  try{ tracker?.markError && tracker.markError(); } catch (e) { void e; }
  ...
  const imagesMap = {1: kaddnum1, 2: kaddid1, 3: kaddnum2, 4: kaddnum2, 5: kaddnum3, 6: kaddnum3, 7: kaddnum5 };
    ...
    <div className={frameStyles.framePage}>
      ...
      <header className={frameStyles.frameHeader} ref={headerRef}>
        <h1 className={`${frameStyles.frameTitle} ${lt.withAccent}`}>
          <span className="titleText">친구 추가하기 (전화번호)</span>
          <span className={frameStyles.inlineTagline}>친구를 전화번호로 추가하는 방법을 연습합니다.</span>
        </h1>
      </header>
